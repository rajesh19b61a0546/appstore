...
class AppStore extends Component {
  state = {initialList: tabsList.tabId, searchValue: ''}

  tabInfo = raj => {
    this.setState({initialList: raj})
  }
  ...
  render() {
    ...
    return (
      <div className="bg">
        ...
        <ul className="u1">
          {onDelete.map(eachItem => (
            <AppItem key={eachItem.appId} eachItem={eachItem} />
          ))}
        </ul>
      </div>
    )
